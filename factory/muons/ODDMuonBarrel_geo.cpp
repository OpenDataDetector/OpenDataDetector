...
static Ref_t create_element(Detector &oddd, xml_h xml, SensitiveDetector sens)
{
	...
	for (xml_coll_t layer(x_det, _U(layer)); layer; ++layer)
	{
		...
		for (int i = 0; i < x_l.nphi(); i++)
		{
			...
			Tube driftTubeShape(rtubemin, rtubemax, chBox.x());
			...
			for (int j = 0; j < x_l.nz(); j++)
			{
				...
				for (int nl = 0; nl < x_gas.repeat(); nl++)
				{
					...
					for (int t = 0; t < ntubes; t++)
					{
						...
						PlacedVolume pvtube = chVolume.placeVolume(driftTubeVolume, Transform3D(RotationY(0.5*std::numbers::pi), Position(0, yt, -chBox.z() + (2 * t + 1) * rshellmax + (nl % 2) * rshellmax)));
						chVolume.placeVolume(driftShellVolume, Transform3D(RotationY(0.5*std::numbers::pi), Position(0, yt, -chBox.z() + (2 * t + 1) * rshellmax + (nl % 2) * rshellmax )));
